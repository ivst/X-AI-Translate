...
Subscription mode is opt-in and requires the separately downloaded [AI Translate Bridge](https://github.com/ivst/X-AI-Translate-Bridge) plus the authenticated provider CLI. Install the bridge bundle, then choose Subscription in Settings. The model selector remains available for OpenAI and Claude; the extension loads the available models from the Bridge and passes the selected model to the corresponding CLI. Gemini and all other providers continue to use their existing API-key flow.
