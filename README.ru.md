...
2. Выберите провайдера. Для Google Translate ключ не нужен, а для DeepL API Free нужен API-ключ DeepL. Для OpenAI и Claude также доступен дополнительный локальный режим подписки, описанный в `bridge/README.md`.
...
Режим подписки включается вручную и требует отдельно скачанного локального bridge и авторизованного CLI провайдера. Откройте документацию bridge, выполните `node server.mjs` из его папки и оставьте терминал открытым во время работы в режиме «Подписка». Gemini и остальные провайдеры продолжают работать через существующий режим API-ключа.
