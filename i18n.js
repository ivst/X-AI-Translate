(function (root) {
  ...
  "en": {
    ...
    "subscription_instructions_step_bridge": "From the extension repository root, install the background bridge using the command below.",
    "subscription_instructions_step_connect": "Return here, select Subscription, and click Connect.",
    "subscription_instructions_note": "On Windows and macOS, the bridge starts in the background when you sign in. For manual mode, run node bridge/server.mjs and keep the terminal open. API-key mode remains available at any time.",
  ...
  "ru": {
    ...
    "subscription_instructions_step_bridge": "Из корня репозитория расширения установите фоновый bridge командой ниже.",
    "subscription_instructions_step_connect": "Вернитесь сюда, выберите «Подписка» и нажмите «Подключить».",
    "subscription_instructions_note": "В Windows и macOS bridge запускается в фоне при входе в систему. Для ручного режима выполните node bridge/server.mjs и оставьте терминал открытым. Режим API-ключа всегда доступен.",
